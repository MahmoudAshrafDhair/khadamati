...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">
        <a href="{{route('admin.workers.index')}}" class="text-muted">{{__('aside.worker')}}</a>
    </li>
@stop
    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">{{__('worker.title')}}
{{--                    <span class="text-muted pt-2 font-size-sm d-block">Javascript array as data source</span></h3>--}}
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{route('admin.workers.create')}}" class="btn btn-info font-weight-bolder">
											<span class="svg-icon svg-icon-md">
												<i class="fa fa-plus"></i>
											</span>{{__('worker.add')}}</a>
                <!--end::Button-->
            </div>
        </div>
        <div class="card-body">
            ...
            <table class="table table-striped- table-bordered table-hover table-checkable" id="data-table">
                ...
                <tr>
                    <th>#</th>
                    <th>{{__('worker.image')}}</th>
                    <th>{{__('worker.username')}}</th>
                    <th>{{__('worker.phone')}}</th>
                    <th>{{__('worker.email')}}</th>
                    <th>{{__('worker.service')}}</th>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">{{__('worker.delete')}}</h5>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.delete').click(function (e) {
                ...
                var url = '{{route('admin.workers.destroy')}}';
                var type = "post";
                $.ajax({
                    type: type,
                    url: url,
